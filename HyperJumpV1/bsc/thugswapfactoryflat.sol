...
// File: contracts/swap/ThugswapFactory.sol

pragma solidity =0.6.12;



contract ThugswapFactory is IThugswapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(ThugswapPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        require(tokenA != tokenB, 'Thugswap: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Thugswap: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Thugswap: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(ThugswapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ThugswapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external override {
        require(msg.sender == feeToSetter, 'Thugswap: FORBIDDEN');
        feeTo = _feeTo;
    }
